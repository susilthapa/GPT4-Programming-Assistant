...
def page_config():
    st.set_page_config(page_title="GPT-4 Coding Asssistant",
                       page_icon="computer")
...
def sidebar():
    ...
    languages = sorted(['Python', 'GoLang', 'TypeScript',
                       'JavaScript', 'Java', 'C', 'C++', 'C#', 'R', 'SQL'])
    python_libs = sorted(['SQLite', 'PyGame', 'Seaborn', "Pandas", 'Numpy', 'Scipy',
                         'Scikit-Learn', 'PyTorch', 'TensorFlow', 'Streamlit', 'Flask', 'FastAPI'])
    ...
    with st.sidebar:
        ...
        if st.session_state.user_authenticated:
            with st.expander(label="Settings", expanded=True):
                coding_settings_tab, chatbot_settings_tab = st.tabs(
                    ['Coding', 'ChatBot'])
            ...
            with st.expander(label="Embed Documents", expanded=True):
                ...
                if pdf_docs:
                    if st.button("Process Docs"):
                        ...
                        initial_llm_chain = create_retrieval_chain(
            ...
            with st.expander("Previous Chats", expanded=True):
                selected_thread_id = st.selectbox(
                    label="Previous Thread IDs", options=get_unique_thread_ids(), index=0)
                if st.button("Render Chat"):
                    st.session_state.thread_id = selected_thread_id
                    st.session_state.chat_history = get_all_thread_messages(
                        selected_thread_id)
...
def handle_initial_submit():
    ...
    initial_template = PromptTemplate(
        input_variables=['input', 'language', 'scenario',
                         'scenario_context', 'code_context', 'libraries'],
        ...
        with st.spinner('Generating Response...'):
            if st.session_state.docs_processed:
                llm_input = initial_template.format(input=code_input,
                                                    ...
                                                    libraries=libraries)
                initial_response = initial_llm_chain(
                    {'question': llm_input})['answer']
            else:
...
def handle_user_message():
    chat_template = PromptTemplate(
        input_variables=['input', 'language', 'scenario', 'scenario_context', 'chat_history',
                         'libraries', 'code_input', 'code_context', 'most_recent_ai_message'],
    ...
    if st.session_state.chat_history:
        user_message = st.text_area(
            "Further Questions for Coding AI?", key="user_input", height=60)
        if st.button("Submit Message") and user_message:
            with st.spinner('Generating Response...'):
                ...
                if st.session_state.docs_processed:
                    ...
                    chat_response = chat_llm_chain(
                        {'question': chat_input})['answer']
                else:
...
def main():
    ...
    if st.session_state.user_authenticated:
        if st.session_state.docs_processed:
            deploy_tab, code_tab, docs_tab = st.tabs(
                ['Deployment', 'Source Code', 'Doc Analysis'])
            with docs_tab:
